...
3. **On Machine 2 (ROS2 Bridge):**
    ...
    * _Optional: Update `MQTT_IP` in the `.env` file to the IP address of Machine 2, or leave it as `127.0.0.1`_
...
4. **On Machine 2 (Robot Worker):**
    ...
    * Open `config/nes_robot_worker_config.yml` and update the following:
        ...
        * Update `localWorkerHost` to the IP address of this Machine and `workerId`
...
6. **On another Machine (Humidity Producer):**
    ...
    * _Optional: Update `MQTT_IP` in the `.env` file to the IP address of this Machine, or leave it as `127.0.0.1`_
